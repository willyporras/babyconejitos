...
function normalizeProduct(raw){
  const sizes={};
  Object.entries(raw.sizes || {}).forEach(([s,q])=>{
...
function loadStockData(){
  return new Promise((resolve,reject)=>{
    if(!STOCK_API_URL){
      reject(new Error("Falta configurar la URL de la API de Stock."));
      return;
    }
    const callbackName=`__stockCallback_${Date.now()}`;
    ...
    const timer=setTimeout(()=>{
      ...
      finished=true;
      cleanup();
      reject(new Error("La consulta de Stock tardó demasiado."));
    ...
    window[callbackName]=(payload)=>{
      ...
      if(!payload || payload.ok!==true || !Array.isArray(payload.products)){
        reject(new Error(payload && payload.error ? payload.error : "Respuesta inválida de la API de Stock."));
        return;
      }
      products=payload.products.map(normalizeProduct).filter(p=>p.code);
      resolve(payload);
    };
    ...
    script.onerror=()=>{
      ...
      clearTimeout(timer);
      cleanup();
      reject(new Error("No se pudo conectar con la API de Stock."));
    };
    const sep=STOCK_API_URL.includes("?")?"&":"?";
    script.src=`${STOCK_API_URL}${sep}callback=${encodeURIComponent(callbackName)}&_=${Date.now()}`;
...
function productVisual(p, thumb=false){
  if(p.imageUrl){
    return `<img class="product-image${thumb?" thumb":""}" src="${escapeHtml(p.imageUrl)}" alt="Código ${escapeHtml(p.code)}" loading="lazy" referrerpolicy="no-referrer">`;
  }
  return p.icon || "👟";
}
...
function renderCoverageProducts(){
  ...
  box.innerHTML=app.selectedCoverage.matches.map(p=>`
    <div class="product-row">
      <div class="product-thumb">${productVisual(p,true)}</div>
      <div>
  ...
  if(!result.length){
    ...
    gal.innerHTML=result.map(p=>`
      <button class="product-card ${totalStock(p)===0?"zero-stock":""}" data-product="${p.code}">
        <div class="shoe-art">${productVisual(p)}</div>
...
function openProduct(code,origin="screenBuscar"){
  ...
  app.previous=origin;
  document.getElementById("detalleArte").innerHTML=productVisual(p);
  ...
  btnAjustar.disabled=READ_ONLY_MODE;
  btnHistorial.disabled=READ_ONLY_MODE;
...
document.getElementById("btnAjustar").addEventListener("click",()=>{
  if(READ_ONLY_MODE) return toast("Función no disponible en modo solo consulta.");
...
document.getElementById("btnGuardarAjuste").addEventListener("click",()=>{
  if(READ_ONLY_MODE) return toast("Los ajustes están deshabilitados en esta versión.");
...
document.getElementById("btnHistorial").addEventListener("click",()=>{
  if(READ_ONLY_MODE) return toast("Historial no disponible en esta versión de consulta.");
...
setDataStatus("Conectando con Google Sheets…");
loadStockData()
  .then(payload=>{
    initFilters();
    setDataStatus(`Datos reales cargados: ${products.length} códigos.`,"ok");
    const ts=payload.generatedAt ? new Date(payload.generatedAt) : null;
    if(ts && !Number.isNaN(ts.getTime())){
      document.getElementById("dataStatus").title=`Última lectura: ${ts.toLocaleString("es-PE")}`;
    }
  })
  .catch(err=>{
    console.error(err);
    setDataStatus(`Sin conexión de datos: ${err.message}`,"error");
    toast("No se pudieron cargar los datos reales de Stock.");
  });
